...
def connect_es() -> Elasticsearch:
    for i in range(10):
        try:
            ...
            es.info()
            log.info(f"Elasticsearch connecte: {ES_HOST}")
            return es
...
def fetch_recent_ships(es: Elasticsearch, minutes: int = 1440) -> pd.DataFrame:
    ...
    try:
        ...
        log.info(f"{len(df)} enregistrements recuperes ({minutes} dernières minutes)")
        return df
...
def detect_anomalies(df: pd.DataFrame) -> List[Dict]:
    if len(df) < 10:
        log.warning("Pas assez de donnees pour la detection d anomalies (min 10)")
        return []
    ...
    if os.path.exists(model_path):
        ...
        log.info("Modele IsolationForest charge depuis disque")
    else:
        pipeline = Pipeline([
        ...
        log.info("Modele IsolationForest entraine et sauvegarde")
    ...
    for idx in anomaly_indices:
        ...
        if speed > 25:
            ...
            severity     = "HIGH"
            description  = f"Vitesse excessive detectee: {speed:.1f} noeuds (normal < 25 kts)"
        elif speed < 0.5 and row.get("nav_status", "") != "Anchored":
            anomaly_type = "VESSEL_STOPPED"
            severity     = "MEDIUM"
            description  = f"Navire a l arret inattendu: {speed:.1f} noeuds"
        else:
            anomaly_type = "UNUSUAL_PATTERN"
            severity     = "LOW"
            description  = f"Comportement inhabituel detecte (score: {score:.3f})"
    ...
    log.info(f"Anomalies detectees: {len(anomalies)} / {len(df)} navires analyses")
    return anomalies
...
def predict_movement(df: pd.DataFrame) -> List[Dict]:
    ...
    log.info(f"Predictions generees: {len(predictions)} navires")
    return predictions
...
def index_results(es: Elasticsearch, index: str, docs: List[Dict], id_field: str = "mmsi"):
    ...
    try:
        ...
        log.info(f"{success}/{len(docs)} documents indexes dans '{index}'")
    except Exception as e:
...
def print_anomaly_report(anomalies: List[Dict]):
    ...
    log.info(f"\n{'='*55}")
    log.info(f"  RAPPORT ANOMALIES ({len(anomalies)} detectees)")
    log.info(f"{'='*55}")
    for a in sorted(anomalies, key=lambda x: x["anomaly_score"], reverse=True)[:10]:
        log.info(
            f"  [{a['severity']:6s}] {a['ship_name']:25s} | "
...
def run():
    log.info(f"ML Engine | ES: {ES_HOST} | intervalle: {RUN_INTERVAL_SEC}s")
        ...
        while True:
            ...
            if not df.empty:
                ...
                if predictions:
                    ...
                    log.info(f"{len(predictions)} predictions de position indexees")
            else:
                log.warning("Aucune donnee recente dans Elasticsearch")

            log.info(f"Prochain cycle dans {RUN_INTERVAL_SEC}s...")
            time.sleep(RUN_INTERVAL_SEC)

    except KeyboardInterrupt:
        log.info("ML Engine arrete")
